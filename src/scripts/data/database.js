import { openDB } from "idb";

const DATABASE_NAME = 'stringstory';
const DATABASE_VERSION = 1;
const OBJECT_STORE_NAME = 'pinned-story';

/** 
 * Open connection to IndexedDB at browser
*/
const idbPromise = openDB(DATABASE_NAME, DATABASE_VERSION, {
	upgrade: (database) => {
		database.createObjectStore(OBJECT_STORE_NAME, {
			keyPath: 'id',
		})
	}
})


const Database = {
	async storeStory(story) {
		if (!Object.hasOwn(story, 'id')) {
			throw new Error('`id` is required to save.')
		}

		return (await idbPromise).put(OBJECT_STORE_NAME, story)
	},

	async removeStory(id) {
		return (await idbPromise).delete(OBJECT_STORE_NAME, id);
	},

	async getStoryById(id) {
		if (!id) {
			throw new Error('`id` is required.');
		}
		return (await idbPromise).get(OBJECT_STORE_NAME, id);
	},

	async getAllStory() {
		return (await idbPromise).getAll(OBJECT_STORE_NAME);
	}
}

export default Database;